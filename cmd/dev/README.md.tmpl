...
A custom build tool for the `{{.project_name}}` project that simplifies common development tasks including building, testing, and deploying the application.
...
**Output:**
- Native builds: `dist/{{.project_name}}`
...
```bash
# Initialize changelog tooling (one-time setup)
./dev changelog init
...
**Subcommands:**
- `init`: Initialize changelog tooling (install git-chglog and setup git hooks)
- `generate`: Generate or update CHANGELOG.md from git history
...
**Prerequisites:**
Run `./dev changelog init` first to install `git-chglog` and setup git hooks:
```bash
./dev changelog init
# or
...
The version is injected into the `{{.go_module}}/pkg/config` package by default.
...
Verify the config package path matches your project structure. The dev tool injects the version into `{{.go_module}}/pkg/config`. If your package structure is different, you'll need to update the `ConfigPackage` field in the build command.
...
3. Use `cmd.Flags()` to retrieve flag values (not `args` by index)
...
This tool is part of the {{.project_name}} project and shares the same license.
